  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/contact", {
        ...
        body: JSON.stringify({ name: form.name.trim(), email: form.email.trim(), msg: form.msg.trim() }),
      })
          ...
          <form className={"contact-form" + (shake ? " shake" : "")} onSubmit={onSubmit}>
            {!sent ? (
              <>
                ...
                <div className="field">
                  <label>MENSAJE</label>
                  <textarea rows={5} value={form.msg} onChange={(e) => setForm({ ...form, msg: e.target.value })} placeholder="Cuéntanos qué tienes en mente…" />
                </div>
              ...
              <div className="terminal-success">
                ...
                <div className="term-body">
                  ...
                  {error ? (
                    <div className="line" style={{ color: "var(--magenta)", marginTop: 12 }}>
                      &gt; ERROR: {error}<span className="caret">_</span>
                    </div>
                  ) : (
                  ...
                  <div style={{ marginTop: 18 }}>
                    <button
                      className="btn ghost"
                      type="button"
                      onClick={() => { setSent(null); setError(null); setForm({ name: "", email: "", msg: "" }) }}
                    >
                      ENVIAR OTRO MENSAJE
                    </button>
                  </div>
